...
public class X_CurvedPlane_Dynamics : MonoBehaviour
{
    // Objects in UNITY
    ...
    void UpdateCurvedDynamics()
    {
        ...
        hypotenuse = new Vector2(y, coefficient_curve * y * y); // y, z

        ro_adjacent_side[0] = adjacent_side[0] * Mathf.Cos(theta_x) - adjacent_side[1] * Mathf.Sin(theta_x);
        ro_adjacent_side[1] = adjacent_side[0] * Mathf.Sin(theta_x) + adjacent_side[1] * Mathf.Cos(theta_x);

        ro_hypotenuse[0] = hypotenuse[0] * Mathf.Cos(theta_x) - hypotenuse[1] * Mathf.Sin(theta_x);
        ro_hypotenuse[1] = hypotenuse[0] * Mathf.Sin(theta_x) + hypotenuse[1] * Mathf.Cos(theta_x);

        curve_theta = Mathf.Atan2(hypotenuse[0] - rb * rb / y, hypotenuse[1]);
        if (Mathf.Pow(y * y, 0.5f) < 0.1f) curve_theta = 0f;
        ...
        else
        {
            ...
            //////////////////////////////////////////
            // tmp[0] = init[0];
            // tmp[2] = Mathf.Cos(theta_X) * (L - init[2]) - Mathf.Sin(theta_X) * init[1];
            // tmp[1] = Mathf.Sin(theta_X) * (L - init[2]) + Mathf.Cos(theta_X) * init[1];
