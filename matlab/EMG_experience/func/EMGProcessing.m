function processedEMG = EMGProcessing(emg)
    ...
    %tiledlayout(4,4)
    for i = 1:numCh
        Voltage = (emgData(:,i)-mean(emgData(1:ceil(length(emgData(:,i))/partition),i)));
        ...
        LPfiltered_EMG(i,:) = lowpass(absVoltage, 1, 50);
        %hf = plot(LPfiltered_EMG','g'); hold on;

        movingRMS(absVoltage,20);
